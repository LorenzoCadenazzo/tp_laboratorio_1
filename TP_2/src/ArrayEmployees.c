...
int employe_modificar(eEmployee array[], int limite)
{
    ...
    if(array!=NULL && limite>0)
    {
        ...
        if(findEmployeeById(array,limite,id,&posicion)==-1)// Si retorna -1 no existe
        {
            printf("\nNo existe este ID");
        }
        else
...
/** \brief finds an Employee by Id and returns the index position in the array.
*
* \param list Employee*
* \param limite int
* \param id int
* \return Returns employee's index position or (-1) if Invalid length or NULL pointer received or employee not found
*
*/
